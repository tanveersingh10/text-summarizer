...
class ConfigurationManager:
    def __init__(self, config_filepath = CONFIG_FILE_PATH, params_filepath = PARAMS_FILE_PATH):
        ...
        self.config = read_yaml(config_filepath) #points to the yaml file
        self.params = read_yaml(params_filepath)

        create_directories([self.config.artifacts_root]) #to store the data files 
    ...
    def get_model_trainer_config(self) -> ModelTrainerConfig:
        config = self.config.model_trainer #from config.yaml
        params = self.params.TrainingArguments #from params.yaml
